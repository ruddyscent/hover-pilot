...
class HoverTaskProfile(str, Enum):
    ...
    def reward_profile(self) -> str:
        ...
        if self is HoverTaskProfile.ELEVATOR_THROTTLE:
            return REWARD_PROFILE_ELEVATOR_THROTTLE
    ...
    @property
    def anchor_roll_to_reset_state(self) -> bool:
        return self in {
            HoverTaskProfile.AILERON,
            HoverTaskProfile.AILERON_THROTTLE,
        }
    ...
    @property
    def uses_connection_episode_boundaries(self) -> bool:
        return self in {
            HoverTaskProfile.AILERON,
            HoverTaskProfile.RUDDER,
            HoverTaskProfile.THROTTLE,
...
def elevator_throttle_features_to_observation(
    elevator_features: ElevatorHoverFeatures,
    *,
    config: RewardConfig,
) -> np.ndarray:
    ...
    # The shared elevator feature uses RealFlight's down-positive world W.
    # Throttle control uses an up-positive vertical velocity.
    observation[-1] *= -1.0
    return observation


def aileron_features_to_observation(
    ...
    *,
    ...
    observation = np.asarray(
        [
            aileron_features.roll_error_deg / config.roll_error_scale_deg,
            aileron_features.roll_rate_deg_s / config.roll_rate_scale_deg_s,
        ],
        dtype=np.float32,
    )
    return np.clip(observation, -5.0, 5.0).astype(np.float32, copy=False)
...
def rudder_features_to_observation(
    ...
    *,
    ...
    observation = np.asarray(
        [
            ...
            rudder_features.yaw_rate_deg_s / config.yaw_rate_scale_deg_s,
        ],
        dtype=np.float32,
    )
    return np.clip(observation, -5.0, 5.0).astype(np.float32, copy=False)
...
def aileron_throttle_features_to_observation(
    aileron_features: AileronHoverFeatures,
    throttle_features: ThrottleHoverFeatures,
    *,
    config: RewardConfig,
...
class HoverPilotHoverEnv(gym.Env):
    ...
    def _compute_rudder_features(
        self,
        state: FlightAxisState,
    ) -> RudderHoverFeatures:
        physics_time_s = state.m_currentPhysicsTime_SEC
        previous_time_s = self._last_rudder_feature_physics_time_s
        if previous_time_s is not None:
            ...
            if delta_time_s > self.physics_time_reset_tolerance_s:
                self._rudder_angle_error_deg = angular_error_deg(
                    self._rudder_angle_error_deg
                    + state.m_yawRate_DEGpSEC * delta_time_s,
                    0.0,
                )
            elif delta_time_s < -self.physics_time_reset_tolerance_s:
                self._rudder_angle_error_deg = 0.0
        self._last_rudder_feature_physics_time_s = physics_time_s
        return compute_rudder_hover_features(
            state,
    ...
    def _state_to_observation(
        ...
        *,
        ...
        if self.task_profile == ELEVATOR_THROTTLE_HOVER_TASK:
            features = (
                elevator_features
        ...
        if self.task_profile == AILERON_THROTTLE_HOVER_TASK:
            roll_features = (
                aileron_features
            ...
            return aileron_throttle_features_to_observation(
                roll_features,
                height_features,
                config=self.reward_config,
            )
        if self.task_profile == RUDDER_HOVER_TASK:
            ...
            return rudder_features_to_observation(
                features,
                config=self.reward_config,
            )
        if self.task_profile == AILERON_HOVER_TASK:
            features = (
                aileron_features
                if aileron_features is not None
                else compute_aileron_hover_features(
            ...
            return aileron_features_to_observation(
                features,
                config=self.reward_config,
            )
    ...
    def step(self, action: np.ndarray):
        ...
        throttle_features = (
            ...
            if self.task_profile.uses_throttle_features
            else None
        )
    ...
    def _start_episode_from_state(self, state: FlightAxisState, episode_start_reason: str):
        ...
        if (
            self.task_profile.anchor_roll_to_reset_state
            and should_anchor_episode_frame
        ):
            self.reward_config = replace(
                self.reward_config,
    ...
    def _is_inactive_reset_state(self, state: FlightAxisState) -> bool:
        if self.task_profile.uses_connection_episode_boundaries:
            # Single-axis Hover Trainer modes remain valid while stationary and
            # may not emit a simulator reset. A deliberate RFLink reconnect is
            # the episode boundary for these profiles.
            return False
        return (
